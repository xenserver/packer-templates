...
cat >> /mnt/zroot/etc/rc.local << EOT
dhclient xn0
EOT
...
# allow root ssh
sed -i'' -e 's/^#PermitRootLogin no/PermitRootLogin yes/' /mnt/zroot/etc/ssh/sshd_config

# zfs doesn't use an fstab, but some rc scripts expect one
touch /mnt/zroot/etc/fstab

# Set up user accounts
echo "vmpassword" | pw -V /mnt/zroot/etc mod user root -h 0
